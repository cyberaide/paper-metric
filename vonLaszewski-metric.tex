...
\section{Towards a Taxonomy for Cloud Metrics}

Cloud metrics must address a wide variety of aspects including which
stakeholders and roles to address, which frameworks
to address,  which purpose the metrics are designed for, which
concrete resources are targeted, which sources are used for the metric,
and which security aspects are considered. We are summarizing these
influential aspects in Figure \ref{F:taxonomy-1} and explaining them
in more detail. 
