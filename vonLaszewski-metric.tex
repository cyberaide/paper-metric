...
\newcommand{\TITLE}{Cloud Metrics for Resource Providers}
...
\item{Consuming.} Many metrics will assist the users and services to
...
\item{Auditing.} Metrics for auditing support the process to evaluate
...
concerns. Such policies need to include role based restrictions of stakeholders
and metrics to be accessed by them.
...
To complete this factor additional aspects need
to be introduced such as granularity and population.
...
\label{T:metrics}
\bigskip
\begin{tabular}{p{0.1\textwidth}p{0.4\textwidth}p{0.3\textwidth}p{0.1\textwidth}}
...
\item[Description:] This metric shows the total amount of runtime for launched instances
\item[Motivation:] Running hour is important for measuring actual runtime of instances
\item[Samples:] FutureGrid Reports (See attached)
\end{description}
