...
of these resources naturally arise. Thus it is important to not only
...
that delivers such impact metrics for XSEDE, NCAR, and Blue Waters. It
is most natural that this be extended to any cloud resource. In fact
the framework is general enough that it could apply to any resource
framework \cite{las14Impact,las15xsede,las15cluster}. Metrics include:

\fugang{please include some of the metrics'}
=======
Another set of metrics are concerning the scientific impact of using a cloud resource. We 
have been working on processes and methods to evaluate a scientific computing facility 
while using XSEDE as the original study object and extending to other similar facilities 
such as NCAR and bluewater \cite{las14Impact,las15xsede,las15cluster}. Table 
\ref{T:SIMmetrics} lists the general metrics related to the impact of scientific works.

\begin{table*}[htb]
\caption{Metrics related to scientific impact evaluation}
\begin{scriptsize}
\label{T:SIMmetrics}
\bigskip
\begin{center}
...
resource &
This measures the general productivity \\
\hline 
Citation Count &
Measuring how many citations has been received as of the measuring time point &
This shows how the output really impacts the scientific community while the 
publications are being cited \\
\hline
H-index, G-index &
While treating the evaluated entity as if it were a person, using the h-index and 
g-index that were originally defined for a person &
This measures both productivity and the generated impact \\
\hline
\end{tabular}
\end{center}
\end{scriptsize}
...
>>>>>>> 0b8995e2279170c84f56ff5597493fc1c988041e
